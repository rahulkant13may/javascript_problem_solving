...
function iqTest(numbers){
    // ...
    // let matchNo = numbers.match(/\d+/g);
    let matchNo = numbers.split(" ")
    console.log(matchNo
               )
    let num = parseInt(matchNo.join(""))
    
    let oddCount = 0;
    let evenCount = 0;
  
    for(let i in matchNo){   
        if(matchNo[i]%2 == 0) evenCount++
        else oddCount++
    }
    ...
    if(evenCount > oddCount){
      for(let i in matchNo){
        if(matchNo[i]%2 !=0){
          console.log(matchNo[i], parseInt(i) + 1)
          break
        }
      }
    }
